...
class CallService {
  ...
  private _isNotUseNotConnected : boolean = false;
  constructor() {
  ...
  private _addListener() : void {
    ...
    this.callEngine.on("onCallNotConnected", (res) => {
      ...
      if (this._isNotUseNotConnected) return
      this._navigateBackCallPage();
      this.stopFloatWindow();
      this._resetCallStore();
    ...
    this.callEngine.on("onUserReject", (res) => {
      ...
      const isGroupCall = uni.$TUIStore.getData(
        StoreName.CALL,
        NAME.IS_GROUP_CALL,
      );
      if (!isGroupCall) {
        this._isNotUseNotConnected = true;
      }
      if (isGroupCall) {
        return
      }
      ...
      plus.nativeUI.toast(t(toastInfo), { align: 'center', verticalAlign: 'center' });
  ...
  public calls(options : CallsOptions) : void {
    ...
    if (options?.callParams && options?.callParams.chatGroupID !== "") {
      isGroupCall = true
    }
    ...
    const params : CallsOptions = {
      ...
      success: () => {
        ...
        let updateStoreParams = {
          ...
          [NAME.CALL_STATUS]: CallStatus.CALLING,
          [NAME.CALL_MEDIA_TYPE]: options?.mediaType,
      ...
      fail: (errCode : number, errMsg : string) => {
        ...
        if (errCode === -1203) {
          const toastInfo = {
            key: CallTips.REPEATE_CALL,
          };
          plus.nativeUI.toast(t(toastInfo), { align: 'center', verticalAlign: 'center' });
        }
        if (callStatus === CallStatus.CONNECTED || callStatus === CallStatus.CALLING || callRole.value == CallRole.CALLEE) {
          return
        }
  ...
  public setBeautyLevel(beautyLevel : number) : void {
    this.callEngine.setBeautyLevel(beautyLevel);
  }
  ...
  public enableMultiDeviceAbility() : void {
    console.log(`${NAME.PREFIX} enableMultiDeviceAbility`);
    const params = {
      enable: true,
      success: () => {
      ...
      fail: (errCode : number, errMsg : string) => {
        console.log(`${NAME.PREFIX} enableMultiDeviceAbility fail`);
      },
    };
    this.callEngine.enableMultiDeviceAbility(params)
  }
  ...
  private _navigateBackCallPage() : void {
    ...
    const currentPage = pagesList[pagesList.length - 1].route;
    console.log(
      `${NAME.PREFIX} _navigateBackCallPage, currentPage = ${currentPage}, ${currentPage.indexOf(
        "callkit/callPage"
      )}`
    );
  ...
  private resetButtonStatus() : void {
    uni.$TUIStore.update(StoreName.CALL, NAME.IS_LOCAL_BLUR_OPEN, false);
    uni.$TUIStore.update(StoreName.CALL, NAME.IS_LOCAL_CAMERA_OPEN, true);
    uni.$TUIStore.update(StoreName.CALL, NAME.IS_EAR_PHONE, false);
    uni.$TUIStore.update(StoreName.CALL, NAME.IS_LOCAL_MIC_OPEN, true);
    uni.$TUIStore.update(StoreName.CALL, NAME.CURRENT_SPEAKER_STATUS, undefined);
    uni.$TUIStore.update(StoreName.CALL, NAME.CURRENT_CAMERA_IS_OPEN, undefined);
  }

  private _resetCallStore() : void {
    this._stopTimer();
    this.stopFloatWindow();
    this._isNotUseNotConnected = false;
